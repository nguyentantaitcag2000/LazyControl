...
- In cases where text input is needed, disable the mouse feature to type using the set shortcut, which can be chosen flexibly between `Ctrl` + `J`, `Ctrl` + `K`, `Ctrl` + `L`, `Ctrl` + `U`, `Ctrl` + `I`, or `Ctrl` + `O`. Default is `Ctrl` + `J`
